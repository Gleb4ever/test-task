package ru.chernyavskiy.test.task.service;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;
import ru.chernyavskiy.test.task.dto.ResponseDto;
import ru.chernyavskiy.test.task.error.SeatAlreadyReservedException;
import ru.chernyavskiy.test.task.error.SeatNotFoundException;
import ru.chernyavskiy.test.task.mappers.SeatMapper;
import ru.chernyavskiy.test.task.model.Seat;
import ru.chernyavskiy.test.task.repository.SeatRepository;
import ru.chernyavskiy.test.task.wrapper.ResponseWrapper;
...
/**
 * Сервис для работы с запросами касающимися сущности посадочных мест {@link Seat}
 */
@Slf4j
@Service
...
public class SeatService {
    ...
    public ResponseDto getAllByHall(Long hallId) throws SeatNotFoundException {
        ...
        return ResponseWrapper.success(found.stream().map(SeatMapper.INSTANCE::map).collect(Collectors.toList()), "Found");
    }
    ...
    public ResponseDto reserveSeats(Collection<Long> seatIds, Boolean isReserve) {
        ...
        return ResponseWrapper.success(reserveResult, "Reserved");
    }
